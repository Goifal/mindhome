...
def executor(ha_mock):
    with patch(
        "assistant.function_calling.yaml_config",
        {
...
class TestSendIntercom:
    ...
    async def test_direct_room_target(self, executor, ha_mock):
        ...
        with patch.object(
            executor, "_find_speaker_in_room", return_value="media_player.echo_wz"
        ):
            with patch(
                "assistant.function_calling.yaml_config",
                {
...
class TestIsTtsSpeaker:
    ...
    def test_device_class_tv_excluded(self):
        ex = self._make_executor()
        assert (
            ex._is_tts_speaker("media_player.samsung", {"device_class": "tv"}) is False
        )
