...
class TestRoomThermalState:
    ...
    def test_to_dict_preserves_booleans(self):
        s = self._make_state(
            heating_active=True, cooling_active=True, sun_exposure=True
        )
        d = s.to_dict()
...
class TestClimateModelInit:
    ...
    @patch(
        "assistant.climate_model.yaml_config",
        {
            "climate_model": {
                "enabled": False,
    ...
    @patch(
        "assistant.climate_model.yaml_config",
        {"climate_model": {"room_params": {"bad": {"heat_loss_coefficient": 0.03}}}},
    )
    def test_get_params_room_specific_override(self):
        ...
        assert (
            params["heating_power_per_min"]
            == DEFAULT_ROOM_THERMAL["heating_power_per_min"]
        )

    @patch(
        "assistant.climate_model.yaml_config",
        {"climate_model": {"default_params": {"heating_power_per_min": 0.2}}},
    )
...
class TestClimateModelSimulate:
    ...
    def _state(self, **kwargs):
        ...
        defaults = dict(
            room="Wohnzimmer", current_temp=20.0, target_temp=21.0, outdoor_temp=5.0
        )
        defaults.update(kwargs)
        return RoomThermalState(**defaults)
    ...
    def test_heating_raises_temp(self):
        model = self._model()
        state = self._state(
            current_temp=18.0, target_temp=22.0, heating_active=True, outdoor_temp=17.0
        )
    ...
    def test_cooling_lowers_temp(self):
        model = self._model()
        state = self._state(
            current_temp=28.0, target_temp=22.0, cooling_active=True, outdoor_temp=30.0
        )
        result = model.simulate(state, duration_minutes=60)
    ...
    def test_reaches_target_with_heating(self):
        model = self._model()
        state = self._state(
            current_temp=20.0, target_temp=21.0, heating_active=True, outdoor_temp=19.0
        )
...
class TestEstimateComfortTime:
    ...
    def test_already_at_comfort(self):
        result = self._model().estimate_comfort_time(
            self._state(current_temp=21.2), comfort_temp=21.0
        )
...
class TestGetContextHint:
    ...
    def test_no_hint_when_no_issues(self):
        state = self._state(
            current_temp=21.0, target_temp=21.0, windows_open=0, heating_active=False
        )
        hint = self._model().get_context_hint([state])
        assert hint == ""
    ...
    def test_max_three_rooms(self):
        states = [
            self._state(room=f"Room{i}", windows_open=1, heating_active=True)
            for i in range(5)
        ]
...
class TestEstimateLossRate:
    ...
    def test_zero_delta_zero_loss(self):
        loss = self._model()._estimate_loss_rate(
            self._state(current_temp=10.0, outdoor_temp=10.0)
        )
        assert loss == 0.0
...
class TestGenerateDescription:
    ...
    def test_temp_drop_description(self):
        s = self._state()
        desc = self._model()._generate_description(
            s, s, {}, 17.0, -3.0, 60, False, None
        )
    ...
    def test_change_parts_listed(self):
        ...
        desc = self._model()._generate_description(
            s, s, changes, 22.0, 2.0, 60, True, 30
        )
...
class TestClimateModelChangesEdgeCases:
    ...
    def _state(self, **kw):
        ...
        defaults = dict(
            room="Bad", current_temp=20.0, outdoor_temp=5.0, target_temp=21.0
        )
        defaults.update(kw)
        return RoomThermalState(**defaults)
    ...
    def test_cooling_reaches_target(self):
        ...
        model = self._model()
        state = self._state(
            current_temp=25.0, target_temp=22.0, cooling_active=True, outdoor_temp=20.0
        )
        result = model.simulate(state, duration_minutes=120)
        assert result["reaches_target"] is True
        assert result["time_to_target_minutes"] is not None
...
class TestGenerateDescriptionCoverage:
    ...
    def test_open_windows_description(self):
        ...
        s = self._state()
        desc = self._model()._generate_description(
            s, s, {"open_windows": 1}, 19.0, -1.0, 60, False, None
        )
    ...
    def test_heating_off_description(self):
        ...
        s = self._state()
        desc = self._model()._generate_description(
            s, s, {"heating_off": True}, 19.0, -1.0, 60, False, None
        )
    ...
    def test_cooling_on_description(self):
        ...
        s = self._state()
        desc = self._model()._generate_description(
            s, s, {"cooling_on": True}, 19.0, -1.0, 60, False, None
        )
    ...
    def test_cooling_off_description(self):
        ...
        s = self._state()
        desc = self._model()._generate_description(
            s, s, {"cooling_off": True}, 20.0, 0.0, 60, False, None
        )
...
class TestSimulateScenario:
    ...
    def test_all_covers_closed_scenario(self):
        ...
        model = self._model()
        result_open = model.simulate_scenario(
            "heating_off",
            duration_hours=12,
            params={
                "current_temp": 21.0,
        ...
        result_covered = model.simulate_scenario(
            "all_covers_closed",
            duration_hours=12,
            params={
                "current_temp": 21.0,
...
class TestEstimateEnergyCost:
    ...
    def test_heating_off_zero_cost(self):
        ...
        model = self._model()
        result = model.estimate_energy_cost(
            "heating_off",
            24,
            params={
                "outdoor_temp": 5.0,
    ...
    def test_custom_price_per_kwh(self):
        ...
        model = self._model()
        result_default = model.estimate_energy_cost(
            "vacation_3days", 72, params={"outdoor_temp": 5.0}
        )
        result_expensive = model.estimate_energy_cost(
            "vacation_3days", 72, price_per_kwh=0.60, params={"outdoor_temp": 5.0}
        )
        if result_default["kwh_total"] > 0:
            assert result_expensive["cost_eur"] == pytest.approx(
                result_default["cost_eur"] * 2, rel=0.01
            )
...
class TestSimulateEdgeCases:
    ...
    def _state(self, **kw):
        ...
        defaults = dict(
            room="Test", current_temp=20.0, outdoor_temp=5.0, target_temp=21.0
        )
        defaults.update(kw)
        return RoomThermalState(**defaults)
    ...
    def test_sun_and_windows_combined(self):
        ...
        model = self._model()
        state = self._state(
            current_temp=20.0, outdoor_temp=10.0, windows_open=1, sun_exposure=True
        )
        result_both = model.simulate(state, duration_minutes=60)
        state_window_only = self._state(
            current_temp=20.0, outdoor_temp=10.0, windows_open=1, sun_exposure=False
        )
    ...
    def test_multiple_windows_increase_loss(self):
        ...
        model = self._model()
        r1 = model.simulate(
            self._state(windows_open=1, outdoor_temp=0.0), duration_minutes=30
        )
        r3 = model.simulate(
            self._state(windows_open=3, outdoor_temp=0.0), duration_minutes=30
        )
...
class TestWhatIfEdgeCases:
    ...
    def test_what_if_heating_on(self):
        ...
        result = self._model().what_if(
            self._state(outdoor_temp=19.0), "Heizung anschalten"
        )
        assert "final_temp" in result
        assert result["final_temp"] > 20.0
...
class TestEstimateComfortTimeEdgeCases:
    ...
    def test_just_outside_threshold(self):
        ...
        assert (
            result["minutes"] is not None or result["minutes"] is None
