...
class TestDeclarativeToolRegistry:
    @patch("assistant.declarative_tools.TOOLS_FILE", Path("/tmp/nonexistent_declarative_test_empty.yaml"))
...
class TestRegistryCreateToolSuccess:
    ...
    def test_create_valid_entity_comparison(self):
        reg = DeclarativeToolRegistry()
        result = reg.create_tool("temp_diff", {
            "type": "entity_comparison",
    ...
    @patch("assistant.declarative_tools.TOOLS_FILE", Path("/tmp/test_decl_create2.yaml"))
    def test_create_valid_threshold_monitor(self):
        reg = DeclarativeToolRegistry()
        result = reg.create_tool("humidity_check", {
            "type": "threshold_monitor",
...
class TestRegistryListTools:
    ...
    def test_list_tools_returns_all(self):
        reg = DeclarativeToolRegistry()
        reg.create_tool("tool_a", {
            "type": "threshold_monitor",
            "description": "A",
            "config": {"entity": "sensor.a", "thresholds": {"min": 1}},
        })
        reg.create_tool("tool_b", {
            "type": "threshold_monitor",
            "description": "B",
...
class TestRegistryMaxToolsLimit:
    ...
    def test_max_tools_reached(self, mock_max):
        reg = DeclarativeToolRegistry()
        reg.create_tool("t1", {
            "type": "threshold_monitor", "description": "1",
            "config": {"entity": "sensor.a", "thresholds": {"min": 1}},
        })
        reg.create_tool("t2", {
            "type": "threshold_monitor", "description": "2",
            "config": {"entity": "sensor.b", "thresholds": {"min": 1}},
        })
        result = reg.create_tool("t3", {
            "type": "threshold_monitor", "description": "3",
    ...
    def test_update_existing_tool_at_limit(self, mock_max):
        ...
        result = reg.create_tool("t1", {
            "type": "threshold_monitor", "description": "Updated",
...
class TestRegistryValidToolNames:
    ...
    def test_valid_name_with_hyphens(self):
        reg = DeclarativeToolRegistry()
        result = reg.create_tool("my-tool-name", {
            "type": "threshold_monitor", "description": "Test",
            "config": {"entity": "sensor.x", "thresholds": {"min": 10}},
        })
        assert result["success"] is True
    ...
    def test_empty_name_rejected(self):
        reg = DeclarativeToolRegistry()
        result = reg.create_tool("", {"type": "threshold_monitor", "description": "T", "config": {"entity": "s", "thresholds": {"min": 1}}})
        assert result["success"] is False

    @patch("assistant.declarative_tools.TOOLS_FILE", Path("/tmp/nonexistent_declarative_test.yaml"))
    def test_special_chars_rejected(self):
        reg = DeclarativeToolRegistry()
        result = reg.create_tool("tool with spaces!", {"type": "threshold_monitor", "description": "T", "config": {"entity": "s", "thresholds": {"min": 1}}})
        assert result["success"] is False
...
class TestExecMultiEntityFormula:
    ...
    async def test_average_formula(self):
        ...
        config = {"entities": {"temp_a": "sensor.a", "temp_b": "sensor.b"}, "formula": "average"}
...
class TestExecEventCounter:
    ...
    async def test_counts_on_states(self):
        ...
        ha.get_history = AsyncMock(return_value=[
            {"state": "on"}, {"state": "off"}, {"state": "on"}, {"state": "on"},
        ])
        ha.get_state = AsyncMock(return_value={"attributes": {"friendly_name": "Light"}})
...
class TestExecEntityAggregator:
    ...
    async def test_average_aggregation(self):
        ...
        with patch("assistant.declarative_tools.TOOLS_FILE", Path("/tmp/nonexistent_declarative_test.yaml")):
            executor = DeclarativeToolExecutor(ha)
        state_map = {
            "sensor.wz": {"state": "20.0", "attributes": {"friendly_name": "WZ", "unit_of_measurement": "°C"}},
            "sensor.sz": {"state": "22.0", "attributes": {"friendly_name": "SZ", "unit_of_measurement": "°C"}},
        }
        ha.get_state = AsyncMock(side_effect=lambda eid: state_map.get(eid))
    ...
    async def test_min_aggregation(self):
        ...
        with patch("assistant.declarative_tools.TOOLS_FILE", Path("/tmp/nonexistent_declarative_test.yaml")):
            executor = DeclarativeToolExecutor(ha)
        state_map = {
            "sensor.a": {"state": "18.0", "attributes": {"friendly_name": "A", "unit_of_measurement": "°C"}},
            "sensor.b": {"state": "22.0", "attributes": {"friendly_name": "B", "unit_of_measurement": "°C"}},
        }
        ha.get_state = AsyncMock(side_effect=lambda eid: state_map.get(eid))
...
class TestExecTrendAnalyzer:
    ...
    async def test_non_numeric_values_only(self):
        ...
        ha.get_history = AsyncMock(return_value=history)
        ha.get_state = AsyncMock(return_value={"attributes": {"friendly_name": "Broken"}})
...
class TestExecStateDuration:
    ...
    async def test_duration_calculation(self):
        ...
        ha.get_history = AsyncMock(return_value=history)
        ha.get_state = AsyncMock(return_value={"attributes": {"friendly_name": "Heizung"}})
        config = {"entity": "climate.heizung", "target_state": "on", "time_range": "24h"}
...
class TestExecTimeComparison:
    ...
    async def test_yesterday_comparison(self):
        ...
        full_history = [{"state": "19.0"}, {"state": "20.0"}, {"state": "22.0"}, {"state": "23.0"}]
...
class TestExecEntityComparisonOperations:
    ...
    async def test_ratio(self):
        ...
        ha.get_state = AsyncMock(side_effect=[
            {"state": "30.0", "attributes": {"friendly_name": "A", "unit_of_measurement": "W"}},
            {"state": "10.0", "attributes": {"friendly_name": "B", "unit_of_measurement": "W"}},
    ...
    async def test_percentage_change(self):
        ...
        config = {"entity_a": "sensor.new", "entity_b": "sensor.old", "operation": "percentage_change"}
    ...
    async def test_entity_a_unavailable(self):
        executor, ha = self._make_exec()
        ha.get_state = AsyncMock(return_value=None)
        config = {"entity_a": "sensor.a", "entity_b": "sensor.b", "operation": "difference"}
