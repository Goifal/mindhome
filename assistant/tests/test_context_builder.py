...
class TestExtractAlerts:
    ...
    def test_water_leak_alarm(self, builder):
        states = [
            {
                "entity_id": "binary_sensor.water_leak_bad",
                "state": "on",
...
class TestAddRecentFacts:
    ...
    def test_respects_limit(self, builder):
        facts = [{"fact": f"Fakt {i}", "confidence": 0.5} for i in range(10)]
        ...
        states = [
            {
                ...
                "state": "above_horizon",
                "attributes": {
                    "next_rising": "2026-03-20T06:00:00",
...
class TestExtractHouseStatusDomains:
    ...
    @patch(
        "assistant.context_builder.get_entity_annotation",
        return_value={"role": "power_meter"},
    )
    ...
    @patch(
        "assistant.context_builder.get_entity_annotation",
        return_value={"role": "switch", "description": "Kaffeemaschine"},
    )
    @patch("assistant.context_builder.is_entity_hidden", return_value=False)
    @patch("assistant.context_builder.get_mindhome_room", return_value=None)
    ...
    @patch(
        "assistant.context_builder.get_entity_annotation",
        return_value={"role": "switch", "description": "Heizluefter"},
    )
    @patch("assistant.context_builder.is_entity_hidden", return_value=False)
    @patch("assistant.context_builder.get_mindhome_room", return_value=None)
        ...
        states = [
            {
                "entity_id": "calendar.personal",
                "state": "off",
                "attributes": {"message": "Zahnarzt", "start_time": "2026-03-21 10:00"},
            },
        ]
        result = builder._extract_house_status(states)
        assert "calendar" in result
        assert any(
            "nächster" in c.lower() and "Zahnarzt" in c for c in result["calendar"]
        )

    @patch(
        "assistant.context_builder.get_entity_annotation",
        return_value={"role": "window_contact", "description": "Fenster Kueche"},
    )
    @patch("assistant.context_builder.is_entity_hidden", return_value=False)
    @patch("assistant.context_builder.get_mindhome_room", return_value=None)
    ...
    @patch(
        "assistant.context_builder.get_entity_annotation",
        return_value={"role": "outdoor_temp", "description": "Aussentemperatur"},
    )
    @patch("assistant.context_builder.is_entity_hidden", return_value=False)
    @patch("assistant.context_builder.get_mindhome_room", return_value=None)
...
class TestGetRoomOverride:
    ...
    @patch(
        "assistant.context_builder._ROOM_PROFILES",
        {
            "schlafzimmer": {
                "name": "Schlafzimmer",
                "overrides": {
