...
from assistant.light_engine import (
    LightEngine,
    _safe_redis,
    _R_OVERRIDE,
    _R_SLEEP,
...
def engine(ha_client):
    with patch(
        "assistant.light_engine.yaml_config",
        {
            "lighting": {
                "enabled": True,
                "presence_control": {"enabled": True, "auto_on_motion": True},
            },
        },
    ):
...
@pytest.mark.asyncio
@patch(
    "assistant.light_engine.get_room_profiles",
    return_value={
        "rooms": {
            "wohnzimmer": {"light_entities": ["light.wz"], "presence_auto_on": True}
        }
    },
)
...
@patch(
    ...
    return_value={
        "rooms": {
            "wohnzimmer": {
                "light_entities": ["light.wz"],
                "presence_auto_on": True,
                "night_path_light": True,
            }
        }
    },
)
...
@patch(
    "assistant.light_engine.yaml_config",
    {
        "lighting": {
            "enabled": True,
            "bed_sensors": {"enabled": True, "sleep_start_hour": 21},
        },
    },
)
...
@patch(
    "assistant.light_engine.get_room_profiles",
    return_value={"rooms": {"schlafzimmer": {"light_entities": ["light.bed"]}}},
)
@patch(
    "assistant.light_engine.yaml_config",
    {
        "lighting": {
            ...
            "bed_sensors": {
                "enabled": True,
                "wakeup_light": True,
                "wakeup_window_start": 5,
                "wakeup_window_end": 9,
                "wakeup_brightness": 40,
                "wakeup_transition": 120,
            },
...
@patch(
    "assistant.light_engine.get_room_profiles",
    return_value={
        "rooms": {"wohnzimmer": {"light_entities": ["light.wz", "light.wz2"]}}
    },
)
def test_get_room_lights(mock_profiles, engine):
    lights = engine._get_room_lights(
        "wohnzimmer", {"light_entities": ["light.wz", "light.wz2"]}
    )
...
@patch(
    "assistant.light_engine.yaml_config",
    {"lighting": {"presence_control": {"night_start_hour": 22, "night_end_hour": 6}}},
)
...
def test_find_room_for_light(engine):
    rooms = {
        "wohnzimmer": {"light_entities": ["light.wz"]},
        "kueche": {"light_entities": ["light.ku"]},
    }
...
@patch(
    "assistant.light_engine.yaml_config",
    {"lighting": {"enabled": True, "default_transition": 2}},
)
...
def test_get_weather_condition_configured():
    ...
    result = LightEngine._get_weather_condition(
        states, {"weather_entity": "weather.custom"}
    )
    assert result == "rainy"
...
@patch(
    ...
    return_value={
        "rooms": {
            "wohnzimmer": {"light_entities": ["light.wz"], "night_brightness": 20}
        }
    },
)
@patch(
    "assistant.light_engine.yaml_config",
    {
        "lighting": {
            "enabled": True,
            "night_dimming": True,
            "night_dimming_start_hour": 21,
            "night_dimming_transition": 300,
        },
    },
)
async def test_night_dimming_dims_bright_lights(mock_profiles, engine, redis_client):
    ...
    with patch("assistant.light_engine.datetime") as mock_dt:
        ...
        await engine._check_night_dimming(
            {"night_dimming_start_hour": 21, "night_dimming_transition": 300},
            states,
        )
    engine.ha.call_service.assert_called()
...
async def test_night_dimming_skips_already_dim_lights(
    mock_profiles, engine, redis_client
):
...
async def test_night_dimming_skips_manual_override(mock_profiles, engine, redis_client):
    ...
    redis_client.get = AsyncMock(
        side_effect=lambda key: b"1" if "override" in key else None
    )
    states = [
        {"entity_id": "light.wz", "state": "on", "attributes": {"brightness": 200}}
    ]
...
@patch(
    "assistant.light_engine.yaml_config",
    {
        "lighting": {
            "enabled": True,
            "bed_sensors": {"enabled": True, "sleep_dim_transition": 300},
        },
    },
)
...
async def test_apply_night_path_light_no_redis(engine):
    ...
    engine.redis = None
    await engine._apply_night_path_light(
        "flur",
        ["light.flur"],
        {"night_path_brightness": 3, "night_path_timeout_minutes": 5},
    )
    engine.ha.call_service.assert_called_once()
...
@patch(
    "assistant.light_engine.get_room_profiles",
    return_value={"rooms": {"schlafzimmer": {}}},
)
@patch("assistant.light_engine.yaml_config", {"lighting": {"enabled": True}})
async def test_apply_wakeup_light_no_lights_in_room(mock_profiles, engine):
    ...
    await engine._apply_wakeup_light(
        "schlafzimmer", {}, {"wakeup_brightness": 40, "wakeup_transition": 120}
    )
    engine.ha.call_service.assert_not_called()
...
@patch(
    "assistant.light_engine.get_room_profiles",
    return_value={"rooms": {"wohnzimmer": {"light_entities": ["light.wz"]}}},
)
@patch("assistant.light_engine.FunctionExecutor")
@patch(
    "assistant.light_engine.yaml_config",
    {
        "lighting": {
            "enabled": True,
            "dusk_sun_elevation": -2,
            "default_transition": 2,
            "dusk_only_occupied_rooms": False,
        },
    },
)
async def test_dusk_auto_on_triggers_at_sunset(
    mock_fe, mock_profiles, engine, redis_client
):
...
@patch(
    "assistant.light_engine.yaml_config",
    {
        "lighting": {
            "enabled": True,
            "night_dimming_start_hour": 21,
            "presence_control": {"night_end_hour": 6},
        }
    },
)
async def test_weather_boost_increases_brightness_on_rain(
    mock_profiles, engine, redis_client
):
...
async def test_lux_adaptive_adjusts_brightness(mock_profiles, engine, redis_client):
    ...
    engine.ha.get_state = AsyncMock(
        return_value={
            "state": "on",
            "attributes": {"brightness": 255},  # Currently 100%
        }
    )
    states = []
...
@patch(
    "assistant.light_engine.yaml_config",
    {"lighting": {"enabled": True, "lux_adaptive": {"enabled": True}}},
)
async def test_lux_adaptive_skips_when_weather_boost_active(
    mock_profiles, engine, redis_client
):
...
@patch("assistant.light_engine.FunctionExecutor")
@patch("assistant.light_engine.yaml_config", {"lighting": {"enabled": True}})
async def test_on_cover_position_change_brightens_light(
    mock_fe, mock_profiles, engine, redis_client
):
...
@patch("assistant.light_engine.FunctionExecutor")
@patch("assistant.light_engine.yaml_config", {"lighting": {"enabled": True}})
async def test_restore_adaptive_brightness(
    mock_fe, mock_profiles, engine, redis_client
):
...
@patch(
    "assistant.light_engine.yaml_config",
    {"lighting": {"bed_sensors": {"wakeup_window_start": 5, "wakeup_window_end": 9}}},
)
...
def test_is_night_no_config_uses_defaults(engine):
    ...
    with patch(
        ...
        {
            "lighting": {
                "presence_control": {"night_start_hour": 23, "night_end_hour": 5}
            }
        },
    ):
        with patch("assistant.light_engine.datetime") as mock_dt:
...
async def test_check_pathlight_timeout_turns_off_expired(engine, redis_client):
    ...
    redis_client.scan = AsyncMock(
        return_value=(b"0", [b"mha:light:pathlight:light.flur"])
    )
