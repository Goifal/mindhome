...
class TestAddressingEdgeCases:
    ...
    async def test_three_persons_addressing(self, advisor, ha_mock):
        ...
        ha_mock.get_states = AsyncMock(return_value=[
            ...
            {"entity_id": "person.lisa", "state": "home",
             "attributes": {"friendly_name": "Lisa"}},
            {"entity_id": "person.tom", "state": "home",
    ...
    async def test_duplicate_titles_deduplicated(self, advisor, ha_mock):
        ...
        ha_mock.get_states = AsyncMock(return_value=[
            {"entity_id": "person.max", "state": "home",
             "attributes": {"friendly_name": "Max"}},
            {"entity_id": "person.max2", "state": "home",
             "attributes": {"friendly_name": "Max"}},
        ])
        with patch("assistant.wellness_advisor.yaml_config", {
            "persons": {"titles": {"max": "Sir"}},
...
class TestMealTimeReminders:
    ...
    @pytest.mark.asyncio
    async def test_meal_reminder_sent_when_due(self, advisor, redis_mock, activity_mock, ha_mock):
    ...
    @pytest.mark.asyncio
    async def test_meal_already_reminded_today(self, advisor, redis_mock, ha_mock, activity_mock):
    ...
    @pytest.mark.asyncio
    async def test_kitchen_active_skips_reminder(self, advisor, redis_mock, ha_mock, activity_mock):
    ...
    @pytest.mark.asyncio
    async def test_user_away_skips_reminder(self, advisor, redis_mock, ha_mock, activity_mock):
    ...
    @pytest.mark.asyncio
    async def test_meal_stressed_mood_short_message(self, advisor, redis_mock, ha_mock, activity_mock, mood_mock):
    ...
    @pytest.mark.asyncio
    async def test_meal_good_mood_message(self, advisor, redis_mock, ha_mock, activity_mock, mood_mock):
    ...
    @pytest.mark.asyncio
    async def test_kitchen_sensor_error_continues(self, advisor, redis_mock, ha_mock, activity_mock):
    ...
    @pytest.mark.asyncio
    async def test_activity_detection_failure_continues(self, advisor, redis_mock, ha_mock, activity_mock):
...
class TestLateNightCheck:
    ...
    @pytest.mark.asyncio
    async def test_nudge_at_2am(self, advisor, redis_mock, activity_mock, ha_mock, mood_mock):
    ...
    @pytest.mark.asyncio
    async def test_cooldown_prevents_second_nudge(self, advisor, redis_mock, activity_mock):
    ...
    @pytest.mark.asyncio
    async def test_activity_detection_failure_returns_early(self, advisor, redis_mock, activity_mock):
    ...
    @pytest.mark.asyncio
    async def test_consecutive_nights_escalation(self, advisor, redis_mock, activity_mock, ha_mock, mood_mock):
...
class TestLLMRewriteNudge:
    ...
    async def test_llm_disabled_returns_original(self, advisor):
        ...
        with patch("assistant.wellness_advisor.yaml_config", {"wellness": {"llm_rewrite": False}}):
            result = await advisor._llm_rewrite_nudge("Test message", "pc_break")
        assert result == "Test message"
    ...
    async def test_llm_rewrite_success(self, advisor):
        advisor._ollama = AsyncMock()
        advisor._ollama.chat = AsyncMock(return_value={
            "message": {"content": "Rewritten wellness message here."}
        })
        with patch("assistant.wellness_advisor.yaml_config", {"wellness": {"llm_rewrite": True}}), \
...
class TestAmbientSuggestionExtended:
    ...
    async def test_suggest_micro_break_cooking(self, advisor):
        ...
        assert result is not None
        assert "kocht" in result.lower() or "wasser" in result.lower() or "setzen" in result.lower()
