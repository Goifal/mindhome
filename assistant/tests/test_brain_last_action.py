...
class TestPerPersonActionTracking:
    ...
    @pytest.mark.asyncio
    async def test_different_persons_isolated(self, brain):
        """Actions are isolated between persons."""
        await brain._set_last_action("set_light", {"room": "wohnzimmer"}, "julia")
        await brain._set_last_action("set_cover", {"room": "schlafzimmer"}, "tobias")

        action_j, args_j = await brain._get_last_action("julia")
        action_t, args_t = await brain._get_last_action("tobias")
    ...
    @pytest.mark.asyncio
    async def test_no_cross_user_leakage(self, brain):
        """Person B should NOT see Person A's action."""
        await brain._set_last_action("set_light", {"room": "buero"}, "julia")
        action, args = await brain._get_last_action("tobias")
        assert action == ""
        assert args == {}

    @pytest.mark.asyncio
    async def test_ttl_expiry(self, brain):
        ...
        result_action, result_args = await brain._get_last_action("julia")
    ...
    @pytest.mark.asyncio
    async def test_clear_action(self, brain):
        """Setting empty action clears the entry."""
        await brain._set_last_action("set_light", {"room": "wohnzimmer"}, "julia")
        await brain._set_last_action("", {}, "julia")
        action, args = await brain._get_last_action("julia")
        assert action == ""

    @pytest.mark.asyncio
    async def test_case_insensitive_person(self, brain):
        """Person keys are case-insensitive."""
        await brain._set_last_action("set_light", {"room": "wohnzimmer"}, "Julia")
        action, _ = await brain._get_last_action("julia")
        assert action == "set_light"

    @pytest.mark.asyncio
    async def test_default_person(self, brain):
        """Empty person defaults to 'user'."""
        await brain._set_last_action("set_light", {"room": "wohnzimmer"}, "")
        action, _ = await brain._get_last_action("")
        assert action == "set_light"
        action2, _ = await brain._get_last_action("user")
        assert action2 == "set_light"

    @pytest.mark.asyncio
    async def test_args_are_copied(self, brain):
        """Args dict should be a copy, not a reference."""
        original_args = {"room": "wohnzimmer", "state": "on"}
        await brain._set_last_action("set_light", original_args, "julia")
        original_args["room"] = "manipulated"  # Mutate original
        _, stored_args = await brain._get_last_action("julia")
