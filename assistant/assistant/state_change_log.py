...
class StateChangeLog:
    ...
    def get_recent(self, n: int = 10, domain: str = "") -> list[dict]:
        ...
        if domain:
            filtered = [
                e for e in self._log if e.get("entity_id", "").startswith(f"{domain}.")
            ]
    ...
    def _get_entity_role(entity_id: str) -> str:
        ...
        try:
            ...
            logger.debug(
                "Entity-Rolle Abfrage fehlgeschlagen fuer %s: %s", entity_id, e
            )
    ...
    def format_conflicts_for_prompt(self, states: dict) -> str:
        ...
        critical_entries = [e for e in all_entries if e[0][2] == "critical"]
        other_entries = [e for e in all_entries if e[0][2] != "critical"]
        ...
        try:
            ...
            if target_entity and active:
                ...
                relevant = [
                    c
                    for c in active
                    if domain in c.get("affected_role", "")
                    or domain in c.get("trigger_role", "")
                ]
                _sev = {"critical": 0, "high": 1, "info": 2}
                relevant.sort(key=lambda c: _sev.get(c.get("severity", "info"), 2))
                for c in relevant[:3]:
    ...
    def format_automations_for_prompt(
        ...
        if lines and len(lines) <= 15:
            parts.append("Aktive Automationen:\n" + "\n".join(lines))
