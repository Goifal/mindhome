...
NAV_STATUS = {"status", "wo bin ich", "übersicht", "übersicht", "wo stehe ich"}
...
class RepairPlanner:
    ...
    async def _restore_session(self):
        ...
        try:
            ...
            logger.info(
                "Workshop-Session wiederhergestellt: %s (Schritt %d/%d)",
                self._session.title,
    ...
    async def create_project(
        self, title, description="", category="maker", priority="normal"
    ) -> dict:
    ...
    async def list_projects(self, status_filter=None, category_filter=None) -> list:
        ...
        if not ids:
            return []
        decoded_ids = [pid.decode() if isinstance(pid, bytes) else pid for pid in ids]
    ...
    async def update_project(self, project_id, **kwargs) -> Optional[dict]:
        ...
        if "status" in kwargs:
            ...
            if old:
                ...
                await self.redis.srem(f"mha:repair:projects:status:{old}", project_id)
        ...
        for k, v in kwargs.items():
            if isinstance(v, (list, dict)):
                v = json.dumps(v)
            await self.redis.hset(f"mha:repair:project:{project_id}", k, str(v))
    ...
    async def complete_project(self, project_id, notes="") -> Optional[dict]:
        ...
        if self.generator:
            try:
                model = self.model_router.model_smart if self.model_router else None
                await self.generator.generate_documentation(project_id, model=model)
            except Exception as e:
    ...
    async def add_project_note(self, project_id, note) -> dict:
        ...
        notes.append(
            {"text": note, "timestamp": datetime.now(timezone.utc).isoformat()}
        )
    ...
    async def add_part(
        self, project_id, name, quantity=1, estimated_cost=0, source=""
    ) -> dict:
    ...
    def _format_diagnosis(self, data: dict) -> str:
        ...
        if steps:
            parts.append(
                f"\n{len(steps)} Schritte erstellt. Sage 'weiter' um zu beginnen."
            )
        return "\n".join(parts)
    ...
    async def simulate_design(self, project_id, question, model=None) -> str:
        ...
        if self.generator:
            ...
            for f in files[:5]:
                content = await self.generator.read_file(project_id, f["name"])
                if content:
...
Erklaere den Fehler und schlage einen konkreten Fix vor."""
    ...
    async def evaluate_measurement(
        self, project_id, measurement_text, model=None
    ) -> str:
...
Gib konkrete Werte, Pruefschritte und erwartete Ergebnisse an."""
    ...
    def _format_step(self, step: RepairStep) -> str:
        ...
        parts = [
            f"Schritt {step.number} von {self._session.total_steps}: {step.title}",
        ]
    ...
    async def scan_object(
        self, image_data=None, camera_name="", description=""
    ) -> dict:
        ...
        if camera_name and self.camera_manager:
            result = await self.camera_manager.get_camera_view(camera_name=camera_name)
        ...
        if image_data and self.ollama:
            ...
            vision_model = yaml_config.get("cameras", {}).get("vision_model", "llava")
            prompt = (
    ...
    async def add_workshop_item(
        self, name, quantity=1, category="werkzeug", location=""
    ) -> dict:
    ...
    async def add_maintenance_schedule(
        self, tool_name, interval_days, last_done=""
    ) -> dict:
    ...
    async def add_expense(self, project_id, item, cost) -> dict:
        ...
        await self.redis.hset(
            f"mha:repair:project:{project_id}", "expenses", json.dumps(expenses)
        )
    ...
    async def return_tool(self, tool_name) -> dict:
        ...
        return {"status": "ok", "message": f"{tool_name} zurückgegeben"}
    ...
    async def list_lent_tools(self) -> list:
        ...
        if keys:
            ...
            for data in all_data:
                data = {
                    dk.decode() if isinstance(dk, bytes) else dk: dv.decode()
    ...
    async def get_printer_status(self) -> dict:
        ...
        if not printer_cfg.get("enabled"):
            return {"status": "disabled", "message": "3D-Drucker nicht aktiviert"}
        prefix = printer_cfg.get("entity_prefix", "octoprint")
        try:
            states = await self.ha.get_states()
        except Exception as e:
            logger.debug("HA-Verbindung fehlgeschlagen: %s", e)
            return {"status": "error", "message": "Home Assistant nicht erreichbar"}
    ...
    async def start_print(self, project_id="", filename="") -> dict:
        ...
        if status.get("status") in ("disabled", "not_found", "error"):
            return status
        if status.get("state") not in ("idle", "ready", "operational", None):
            return {
                "status": "error",
                "message": f"Drucker nicht bereit: {status.get('state')}",
            }
        ws_cfg = yaml_config.get("workshop", {})
        prefix = ws_cfg.get("printer_3d", {}).get("entity_prefix", "octoprint")
        try:
            await self.ha.call_service(
                "button", "press", {"entity_id": f"button.{prefix}_resume_job"}
            )
        except Exception as e:
    ...
    async def pause_print(self) -> dict:
        ...
        try:
            await self.ha.call_service(
                "button", "press", {"entity_id": f"button.{prefix}_pause_job"}
            )
        except Exception as e:
    ...
    async def cancel_print(self) -> dict:
        ...
        try:
            await self.ha.call_service(
                "button", "press", {"entity_id": f"button.{prefix}_cancel_job"}
            )
        except Exception as e:
            logger.error("3D-Druck Abbruch fehlgeschlagen: %s", e)
            return {
                "status": "error",
                "message": "Druck konnte nicht abgebrochen werden",
            }
    ...
    async def _arm_command(self, cmd: dict) -> dict:
        ...
        if not arm_cfg.get("enabled"):
            return {"status": "disabled", "message": "Roboterarm nicht aktiviert"}
        url = arm_cfg.get("url", "")
        if not url:
            return {"status": "error", "message": "Arm-URL nicht konfiguriert"}
        ...
        try:
            ...
            if parsed.scheme not in ("http", "https"):
                ...
                return {"status": "error", "message": "Arm-URL ungueltig"}
            # F-082: Userinfo blockieren
            if parsed.username or parsed.password or "@" in (parsed.netloc or ""):
                return {"status": "error", "message": "Arm-URL ungueltig (Userinfo)"}
            ...
            if not hostname:
                return {"status": "error", "message": "Arm-URL ohne Hostname"}
                ...
                try:
                    ...
                    for _fam, _type, _proto, _cname, sockaddr in infos or []:
                        try:
                            ...
                            if (
                                isinstance(resolved, ipaddress.IPv6Address)
                                and resolved.ipv4_mapped
                            ):
                                resolved = resolved.ipv4_mapped
                            for net in _ARM_BLOCKED_NETS:
                                if resolved in net:
                                    logger.warning(
                                        "Arm-URL: DNS '%s' → blockierte IP %s",
                                        hostname,
                                        resolved,
                                    )
                                    return {
                    ...
                    logger.warning(
                        "Arm-URL: DNS-Auflösung fehlgeschlagen für '%s': %s",
                        hostname,
                        e,
                    )
                    return {"status": "error", "message": "Arm-URL nicht erreichbar"}
        except Exception as e:
            logger.error("Arm-URL Validierung fehlgeschlagen: %s", e)
            return {"status": "error", "message": "Arm-URL Validierung fehlgeschlagen"}
        try:
            import aiohttp
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    if "json" not in ct.lower():
                        logger.warning(
                            "Arm-Response: unerwarteter Content-Type '%s'", ct[:80]
                        )
                        return {
                            "status": "error",
                    ...
                    if len(raw) > _MAX_ARM_RESPONSE:
                        return {"status": "error", "message": "Arm-Antwort zu gross"}
    ...
    async def arm_move(self, x, y, z, speed=50) -> dict:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": "Ungueltige Koordinaten oder Geschwindigkeit",
            }
    ...
    async def start_timer(self, project_id) -> dict:
        ...
        if not self.redis:
            return {"status": "error", "message": "Redis nicht verfügbar"}
        await self.redis.hset(
            f"mha:repair:project:{project_id}", "timer_start", str(time.time())
        )
    ...
    async def pause_timer(self, project_id) -> dict:
        ...
        if not self.redis:
            return {"status": "error", "message": "Redis nicht verfügbar"}
        start = float(
            await self.redis.hget(f"mha:repair:project:{project_id}", "timer_start")
            or 0
        )
        total = float(
            await self.redis.hget(f"mha:repair:project:{project_id}", "timer_total")
            or 0
        )
        if start > 0:
            total += time.time() - start
            await self.redis.hset(
                f"mha:repair:project:{project_id}", "timer_total", str(total)
            )
            await self.redis.hset(
                f"mha:repair:project:{project_id}", "timer_start", "0"
            )
    ...
    async def link_device_to_project(self, project_id, entity_id) -> dict:
        ...
        if not self.redis:
            return {"status": "error", "message": "Redis nicht verfügbar"}
        await self.redis.hset(
            f"mha:repair:project:{project_id}", "device_entity", entity_id
        )
        return {"status": "ok"}
    ...
    async def toggle_activation(self, text: str) -> str:
        ...
        if text_lower in ACTIVATION_ON:
            if self.redis:
                await self.redis.setex(
                    "mha:repair:manual_active", self.REDIS_SESSION_TTL, "1"
                )
            _title = yaml_config.get("person", {}).get("title", "Sir")
