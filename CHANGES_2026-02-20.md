...
## 5. Optimierung: Entity-Suche ueber MindHome Device-DB

**Problem:** `_find_entity()` laedt bei jedem Aufruf ALLE HA-Entity-States (kann hunderte sein) nur um ein einzelnes Geraet zu finden. Langsam und verschwenderisch.

**Fix:** Neuer Suchendpoint `GET /api/devices/search?domain=light&room=küche` in MindHome. Die Entity-Suche fragt jetzt ZUERST die MindHome Device-DB ab (schnelle DB-Query mit Domain+Raum Filter). Nur als Fallback werden alle HA-States geladen.

**Dateien:**
- `addon/rootfs/opt/mindhome/routes/devices.py` — Neuer `/api/devices/search` Endpoint
- `assistant/assistant/ha_client.py` — Neue `search_devices()` Methode
- `assistant/assistant/function_calling.py` — `_find_entity()` mit MindHome-First Strategie
