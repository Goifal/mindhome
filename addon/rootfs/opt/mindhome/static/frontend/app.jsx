    ...
    return (
        <div ref={ref} style={{ position: 'relative' }}>
            ...
            <div className="input" onClick={() => setOpen(!open)} style={{
                ...
                <span style={{ color: selected ? 'var(--text-primary)' : 'var(--text-muted)' }}>{selected?.label || placeholder || 'Ã¢â‚¬â€ AuswÃ¤hlen Ã¢â‚¬â€'}</span>
        ...
        <div ref={ref} style={{ position: 'relative' }}>
            ...
            <div className="input" onClick={() => setOpen(!open)} style={{
                ...
                <span style={{ color: displayValue ? 'var(--text-primary)' : 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, fontSize: 13 }}>
                    {displayValue || placeholder || 'Ã¢â‚¬â€ Entity wÃ¤hlen Ã¢â‚¬â€'}
                </span>
                ...
                <div style={{
                    ...
                    <div style={{ padding: '8px 8px 4px' }}>
                        <input ref={inputRef} className="input" value={search} onChange={e => setSearch(e.target.value)}
                            placeholder="Ã°Å¸â€Â Suchen..." style={{ fontSize: 12, padding: '6px 10px' }} />
                    </div>
...
const stateDisplay = (state) => {
    ...
    if (state === 'unavailable') return { label: 'Ã¢Å“â€¢', color: 'var(--danger)' };
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, padding: '0 16px 16px' }}>
                        ...
                        <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 8, textAlign: 'center' }}>
                            <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--success)' }}>
                                {learningStats.avg_confidence ? `${Math.round(learningStats.avg_confidence * 100)}%` : 'Ã¢â‚¬â€'}
                            </div>
                            <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
...
const QuickActionsGrid = () => {
    ...
    const iconOptions = [
        { value: 'mdi:flash', label: 'Ã¢Å¡Â¡ Flash' }, { value: 'mdi:lightbulb', label: 'Ã°Å¸â€™Â¡ Licht' },
        { value: 'mdi:home', label: 'Ã°Å¸ÂÂ  Home' }, { value: 'mdi:exit-run', label: 'Ã°Å¸Å¡Âª Gehen' },
        { value: 'mdi:weather-night', label: 'Ã°Å¸Å’â„¢ Nacht' }, { value: 'mdi:shield', label: 'Ã°Å¸â€ºÂ¡Ã¯Â¸Â Schutz' },
        { value: 'mdi:movie-open', label: 'Ã°Å¸Å½Â¬ Kino' }, { value: 'mdi:broom', label: 'Ã°Å¸Â§Â¹ AufrÃ¤umen' },
        { value: 'mdi:party-popper', label: 'Ã°Å¸Å½â€° Party' }, { value: 'mdi:coffee', label: 'Ã¢Ëœâ€¢ Kaffee' },
    ];
...
const DevicesPage = () => {
    ...
    const getDomainName = (domainId) => domains.find(d => d.id === domainId)?.display_name || 'Ã¢â‚¬â€';
    const getRoomName = (roomId) => rooms.find(r => r.id === roomId)?.name || 'Ã¢â‚¬â€';
            ...
            {devices.length > 0 ? (
                <div>
                    <div style={{ marginBottom: 12 }}>
                        <input className="input" placeholder={lang === 'de' ? 'Ã°Å¸â€Â GerÃ¤te suchen...' : 'Ã°Å¸â€Â Search devices...'}
                    ...
                    <table>
                        ...
                        <tbody>
                            {getFilteredDevices().map(device => {
                                ...
                                const attrParts = [];
                                if (attrs.brightness_pct != null) attrParts.push(`Ã¢Ëœâ‚¬ ${attrs.brightness_pct}%`);
                                if (attrs.position_pct != null) attrParts.push(`Ã¢â€ â€¢ ${attrs.position_pct}%`);
                                if (attrs.current_temp != null) attrParts.push(`Ã°Å¸Å’Â¡ ${attrs.current_temp}${unit || 'Â°C'}`);
                                if (attrs.target_temp != null) attrParts.push(`Ã¢â€ â€™ ${attrs.target_temp}${unit || 'Â°C'}`);
                                if (attrs.humidity != null) attrParts.push(`Ã°Å¸â€™Â§ ${attrs.humidity}%`);
                                if (attrs.power != null || attrs.current_power_w != null) attrParts.push(`Ã¢Å¡Â¡ ${attrs.power || attrs.current_power_w} W`);
                                if (attrs.voltage != null) attrParts.push(`Ã°Å¸â€Å’ ${attrs.voltage} V`);
                ...
                <Modal title={lang === 'de' ? 'GerÃ¤t bearbeiten' : 'Edit Device'} onClose={() => setEditDevice(null)}
                    ...
                    <div className="input-group" style={{ marginBottom: 16 }}>
                        <Dropdown
                            ...
                            options={[
                                { value: '', label: lang === 'de' ? 'Ã¢â‚¬â€ Kein Raum Ã¢â‚¬â€' : 'Ã¢â‚¬â€ No Room Ã¢â‚¬â€' },
                    ...
                    <div className="input-group" style={{ marginBottom: 16 }}>
                        <Dropdown
                            ...
                            options={[
                                { value: '', label: lang === 'de' ? 'Ã¢â‚¬â€ Keine Ã¢â‚¬â€' : 'Ã¢â‚¬â€ None Ã¢â‚¬â€' },
                ...
                <Modal title={lang === 'de' ? `${bulkCount} GerÃ¤te bearbeiten` : `Edit ${bulkCount} Devices`} onClose={() => setShowBulkEdit(false)}
                    ...
                    <div className="input-group" style={{ marginBottom: 16 }}>
                        <Dropdown
                            ...
                            options={[
                                { value: '', label: lang === 'de' ? 'Ã¢â‚¬â€ Nicht Ã¤ndern Ã¢â‚¬â€' : 'Ã¢â‚¬â€ No change Ã¢â‚¬â€' },
                                    ...
                                    <div>
                                        ...
                                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                                            {user.ha_person_entity
                                                ? `Ã°Å¸â€â€” ${user.ha_person_entity}`
                                                : (lang === 'de' ? 'Ã¢Å¡Â Ã¯Â¸Â Keine HA-Person' : 'Ã¢Å¡Â Ã¯Â¸Â No HA person')}
                                        </div>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: 4 }}>
                ...
                <Modal title={lang === 'de' ? `HA-Person: ${editingUser.name}` : `HA Person: ${editingUser.name}`} onClose={() => setEditingUser(null)}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ padding: '10px 14px', borderRadius: 8, cursor: 'pointer',
                            ...
                            {lang === 'de' ? 'Ã¢â‚¬â€ Keine Person Ã¢â‚¬â€' : 'Ã¢â‚¬â€ No Person Ã¢â‚¬â€'}
                        </div>
                        {haPersons.map(p => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, alignItems: 'start' }}>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div className="input-group" style={{ marginBottom: 16 }}>
                    <Dropdown
                        ...
                        options={[
                            { value: 'de', label: 'Ã°Å¸â€¡Â©Ã°Å¸â€¡Âª Deutsch' },
                            { value: 'en', label: 'Ã°Å¸â€¡Â¬Ã°Å¸â€¡Â§ English' },
                        ]}
                    />
                </div>
                ...
                <div className="input-group" style={{ marginBottom: 16 }}>
                    <Dropdown
                        ...
                        options={[
                            { value: 'dark', label: lang === 'de' ? 'Ã°Å¸Å’â„¢ Dunkel' : 'Ã°Å¸Å’â„¢ Dark' },
                            { value: 'light', label: lang === 'de' ? 'Ã¢Ëœâ‚¬Ã¯Â¸Â Hell' : 'Ã¢Ëœâ‚¬Ã¯Â¸Â Light' },
                        ]}
                    />
                </div>
                ...
                <div className="input-group">
                    <Dropdown
                        ...
                        options={[
                            { value: 'simple', label: lang === 'de' ? 'Ã°Å¸â€œâ€¹ Einfach' : 'Ã°Å¸â€œâ€¹ Simple' },
                            { value: 'advanced', label: lang === 'de' ? 'Ã°Å¸â€œÅ  AusfÃ¼hrlich' : 'Ã°Å¸â€œÅ  Advanced' },
                        ]}
                    />
                </div>
            </div>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <InfoRow label="Phase" value={`2 Ã¢â‚¬â€œ ${lang === 'de' ? 'VollstÃ¤ndig' : 'Complete'}`} />
                    <InfoRow label="Home Assistant"
                        value={sysInfo?.ha_connected ? (lang === 'de' ? 'Ã¢Å“â€¦ Verbunden' : 'Ã¢Å“â€¦ Connected') : (lang === 'de' ? 'Ã¢ÂÅ’ Getrennt' : 'Ã¢ÂÅ’ Disconnected')} />
                    <InfoRow label={lang === 'de' ? 'Zeitzone' : 'Timezone'}
                        value={sysInfo?.timezone || 'Ã¢â‚¬â€'} />
                    <InfoRow label={lang === 'de' ? 'HA Entities' : 'HA Entities'}
                        value={sysInfo?.ha_entity_count || 'Ã¢â‚¬â€'} />
                    <InfoRow label={lang === 'de' ? 'DatenbankgrÃ¶ÃŸe' : 'Database Size'}
                        value={sysInfo?.db_size_bytes ? formatBytes(sysInfo.db_size_bytes) : 'Ã¢â‚¬â€'} />
                    <InfoRow label="Uptime"
                        value={sysInfo?.uptime_seconds ? `${Math.floor(sysInfo.uptime_seconds / 3600)} h` : 'Ã¢â‚¬â€'} />
                    <InfoRow label={lang === 'de' ? 'Gesammelte Events' : 'Collected Events'}
                        value={sysInfo?.state_history_count?.toLocaleString() || '0'} />
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button className="btn btn-sm btn-secondary" onClick={async () => {
                        ...
                        if (r) showToast(r.healthy ? (lang === 'de' ? 'Ã¢Å“â€¦ System gesund' : 'Ã¢Å“â€¦ System healthy') : `Ã¢Å¡Â Ã¯Â¸Â ${r.issues?.join(', ')}`, r.healthy ? 'success' : 'warning');
                    }}>
                    ...
                    <button className="btn btn-sm btn-secondary" onClick={async () => {
                        ...
                        if (r) showToast(r.passed ? (lang === 'de' ? 'Ã¢Å“â€¦ Selbsttest bestanden' : 'Ã¢Å“â€¦ Self-test passed') : `Ã¢Å¡Â Ã¯Â¸Â ${r.tests?.filter(t => t.status !== 'ok').map(t => t.test).join(', ')}`, r.passed ? 'success' : 'warning');
                    }}>
                    ...
                    <button className="btn btn-sm btn-secondary" onClick={async () => {
                        ...
                        if (r) showToast(r.update_available ? `Update: ${r.latest_version}` : (lang === 'de' ? `v${r.current_version} Ã¢â‚¬â€œ Aktuell` : `v${r.current_version} Ã¢â‚¬â€œ Up to date`), r.update_available ? 'info' : 'success');
                    }}>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer', marginBottom: 8 }}>
                    ...
                    {lang === 'de' ? 'Historische Daten einschlieÃŸen' : 'Include historical data'}
                </label>
                <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 0 }}>
                    {lang === 'de'
                        ? 'Standard: Konfiguration + Einstellungen. VollstÃ¤ndig: inkl. Muster, Regeln und Logs.'
        ...
        <div className="card" style={{ marginBottom: 16 }}>
            ...
            {triggers.length === 0 && !showAdd ? (
                ...
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    ...
                    <div>
                        <div style={{ fontWeight: 500 }}>{t.keyword} Ã¢â€ â€™ {actionLabels[t.action] || t.action}</div>
            ...
            {loading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {filtered.map(log => {
                        ...
                        const attrParts = [];
                        if (attrs.brightness_pct !== undefined) attrParts.push(`Ã°Å¸â€™Â¡ ${attrs.brightness_pct}%`);
                        if (attrs.position_pct !== undefined) attrParts.push(`Ã¢â€ â€¢ ${attrs.position_pct}%`);
                        if (attrs.target_temp !== undefined) attrParts.push(`Ã°Å¸Å’Â¡ ${attrs.target_temp}Â°C`);
                            ...
                            {auditLogs.map(a => (
                                <div key={a.id} style={{ padding: '8px 0', borderBottom: '1px solid var(--border-color)', fontSize: 13 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        ...
                                        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{a.created_at ? relativeTime(a.created_at, lang) : 'Ã¢â‚¬â€œ'}</span>
                                    </div>
...
const PatternsPage = () => {
    ...
    useEffect(() => {
        // Auto-reclassify existing sensorÃ¢â€ â€™sensor patterns as insights
    ...
    return (
        <div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
                                        ...
                                        Ã¢â€ â€™ <strong>{r.action_entity}</strong> {r.action_service}
            ...
            {(() => {
                ...
                <div className="stat-grid" style={{ marginBottom: 24 }}>
                    ...
                    <div className="stat-card animate-in animate-in-delay-3">
                        ...
                        <div>
                            <div className="stat-value">{s.avg_confidence ? `${Math.round(s.avg_confidence * 100)}%` : 'Ã¢â‚¬â€'}</div>
            ...
            <div className="card animate-in" style={{ marginBottom: 16, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                    {[{ id: 'conservative', label: lang === 'de' ? 'Vorsichtig' : 'Conservative', icon: 'Ã°Å¸ÂÂ¢' },
                      { id: 'normal', label: 'Normal', icon: 'Ã¢Å¡â€“Ã¯Â¸Â' },
                      { id: 'aggressive', label: lang === 'de' ? 'Aggressiv' : 'Aggressive', icon: 'Ã°Å¸Å¡â‚¬' }].map(s => (
                        ...
                        <table className="data-table" style={{ fontSize: 12 }}>
                            <thead>
                                <tr>
                                    ...
                                    <th>Entity</th>
                                    <th>{lang === 'de' ? 'Alt Ã¢â€ â€™ Neu' : 'Old Ã¢â€ â€™ New'}</th>
                ...
                <div className="card animate-in" style={{ marginBottom: 16, borderLeft: '3px solid var(--warning)', padding: '12px 16px' }}>
                    ...
                    {conflicts.slice(0, 3).map((c, i) => (
                        <div key={i} style={{ fontSize: 12, color: 'var(--text-secondary)', padding: '4px 0' }}>
                            {c.message_de || c.message_en || c.description || `${c.pattern_a?.desc || c.pattern_a?.id || '?'} Ã¢â€ â€ ${c.pattern_b?.desc || c.pattern_b?.id || '?'}`}
                        </div>
                    ))}
                </div>
            )}
                            ...
                            {expandedId === p.id && (
                                <div style={{
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px', marginBottom: 12 }}>
                                        <div><span style={{ color: 'var(--text-muted)' }}>Entity:</span> <code style={{ fontSize: 12 }}>{p.pattern_data?.entity_id || p.pattern_data?.action_entity || 'Ã¢â‚¬â€œ'}</code></div>
                                        <div><span style={{ color: 'var(--text-muted)' }}>{lang === 'de' ? 'Zielzustand' : 'Target'}:</span> <strong>{p.pattern_data?.target_state || p.action_definition?.target_state || 'Ã¢â‚¬â€œ'}</strong></div>
                                        {p.pattern_data?.avg_hour !== undefined && (
                                            <div><span style={{ color: 'var(--text-muted)' }}>{lang === 'de' ? 'Uhrzeit' : 'Time'}:</span> <strong>{String(p.pattern_data.avg_hour).padStart(2,'0')}:{String(p.pattern_data.avg_minute||0).padStart(2,'0')}</strong> Ã‚Â±{p.pattern_data.time_window_min || 15}min</div>
                                        )}
                                        {p.pattern_data?.weekday_filter && (
                                            <div><span style={{ color: 'var(--text-muted)' }}>{lang === 'de' ? 'Tage' : 'Days'}:</span> {p.pattern_data.weekday_filter === 'weekdays' ? (lang === 'de' ? 'MoÃ¢â‚¬â€œFr' : 'MonÃ¢â‚¬â€œFri') : p.pattern_data.weekday_filter === 'weekends' ? (lang === 'de' ? 'SaÃ¢â‚¬â€œSo' : 'SatÃ¢â‚¬â€œSun') : (lang === 'de' ? 'Alle' : 'All')}</div>
                                        )}
                                        ...
                                        {p.pattern_data?.trigger_entity && (
                                            <div><span style={{ color: 'var(--text-muted)' }}>Trigger:</span> <code style={{ fontSize: 12 }}>{p.pattern_data.trigger_entity}</code> Ã¢â€ â€™ {p.pattern_data.trigger_state}</div>
                                        )}
                                        ...
                                        <div><span style={{ color: 'var(--text-muted)' }}>{lang === 'de' ? 'Erstellt' : 'Created'}:</span> {p.created_at ? new Date(p.created_at).toLocaleDateString() : 'Ã¢â‚¬â€œ'}</div>
                                    ...
                                    <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                                        ...
                                        {p.season && <span className="badge badge-info" style={{ fontSize: 10 }}>Ã°Å¸Å’Â¿ {p.season}</span>}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div className="card">
                            ...
                            {[
                                { key: 'escalation', icon: 'mdi-arrow-up-bold', de: 'Eskalation (Push Ã¢â€ â€™ TTS)', en: 'Escalation (Push Ã¢â€ â€™ TTS)' },
                                ...
                                <div style={{ display: 'flex', gap: 4 }}>
                                    {[5, 10, 20, 0].map(r => (
                                        <button key={r} className={`btn btn-sm ${(extSettings?.rate_limits?.anomaly || 10) === r ? 'btn-primary' : 'btn-ghost'}`}
                                            ...
                                            style={{ fontSize: 10, padding: '2px 6px' }}>{r === 0 ? 'Ã¢Ë†Å¾' : r}</button>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <button className={`btn ${notifSettings?.dnd_enabled ? 'btn-warning' : 'btn-secondary'}`}
                            ...
                            {notifSettings?.dnd_enabled ? (lang === 'de' ? 'DND aktiv Ã¢â‚¬â€œ Benachrichtigungen stumm' : 'DND active') : (lang === 'de' ? 'Nicht stÃ¶ren aktivieren' : 'Enable Do Not Disturb')}
                        </button>
                    </div>
                </div>
            ) : (<div>
                    ...
                    {step === 1 && (
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: 12 }}>
                                {[{ code: 'de', label: 'Deutsch', flag: 'Ã°Å¸â€¡Â©Ã°Å¸â€¡Âª' }, { code: 'en', label: 'English', flag: 'Ã°Å¸â€¡Â¬Ã°Å¸â€¡Â§' }].map(opt => (
                                    <button key={opt.code}
...
const App = () => {
    ...
    // Role: first user is always admin, or if only 1 user exists Ã¢â€ â€™ admin
