        ...
        <div>
            ...
            {tab === 'config' && (
                <div className="animate-in">
                    ...
                    <div style={{ marginTop: 16 }}>
                        <RoomHumiditySensors lang={lang} showToast={showToast} />
                    </div>
                    <div style={{ marginTop: 16 }}>
...
// ================================================================
// Room Humidity Sensors (analog zu RoomTemperatureSensors)
// ================================================================

const RoomHumiditySensors = ({ lang, showToast }) => {
    const [configured, setConfigured] = useState(null);
    const [available, setAvailable] = useState([]);
    const [showPicker, setShowPicker] = useState(false);
    const [search, setSearch] = useState('');
    ...
    const load = () => {
        api.invalidate('ui/room-humidity');
        api.get('ui/room-humidity').then(d => setConfigured(d || {})).catch(() => {});
    };
    useEffect(() => {
        load();
    }, []);
    ...
    const openPicker = async () => {
        const d = await api.get('ui/room-humidity/available');
        setAvailable(d?.sensors || []);
        setShowPicker(true);
        setSearch('');
    };
    ...
    const addSensor = async (entityId) => {
        ...
        if (current.includes(entityId)) return;
        const updated = [...current, entityId];
        const r = await api.put('ui/room-humidity', { sensors: updated });
        if (r?.success) {
            showToast(lang === 'de' ? 'Sensor hinzugefuegt' : 'Sensor added', 'success');
            load();
        }
    ...
    return (
        <div className="card">
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span className="mdi mdi-water-percent" style={{ color: 'var(--accent-primary)' }} />
                    {lang === 'de' ? 'Luftfeuchtigkeits-Sensoren' : 'Humidity Sensors'}
                </div>
                {configured?.average != null && (
                    <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--accent-primary)' }}>
                        {configured.average}%
            ...
            <div style={{ padding: 16 }}>
                <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
                    {lang === 'de'
                        ? 'Jarvis ueberwacht nur die hier konfigurierten Sensoren auf Luftfeuchtigkeit. Ohne Auswahl werden alle Sensoren geprueft.'
                        : 'Jarvis only monitors the sensors configured here for humidity. Without selection, all sensors are checked.'}
                </div>

                {/* Configured sensors */}
                {(configured?.sensors || []).length === 0 ? (
                    <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                        <span className="mdi mdi-water-off" style={{ fontSize: 28, display: 'block', marginBottom: 6 }} />
                        {lang === 'de'
                            ? 'Noch keine Sensoren konfiguriert. Jarvis prueft aktuell alle Luftfeuchtigkeits-Sensoren.'
                            : 'No sensors configured yet. Jarvis currently checks all humidity sensors.'}
                    </div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {configured.sensors.map(s => (
                            <div key={s.entity_id} style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{
                                        ...
                                        {s.value != null ? `${s.value}%` : (lang === 'de' ? 'n/v' : 'n/a')}
                                    </span>
                                    <button className="btn btn-ghost btn-icon" style={{ color: 'var(--danger)', padding: 4 }}
                                        onClick={() => removeSensor(s.entity_id)}
                                        title={lang === 'de' ? 'Entfernen' : 'Remove'}>
                ...
                {showPicker && (
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span>
                                    <span className="mdi mdi-water-plus" style={{ marginRight: 8, color: 'var(--accent-primary)' }} />
                                    {lang === 'de' ? 'Feuchtigkeitssensor waehlen' : 'Choose Humidity Sensor'}
                                </span>
                                <button className="btn btn-ghost btn-icon" onClick={() => setShowPicker(false)}>
                                    <span className="mdi mdi-close" />
                                </button>
                            ...
                            <div style={{ overflowY: 'auto', flex: 1 }}>
                                {filtered.length === 0 && (
                                    <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)' }}>
                                        {lang === 'de' ? 'Keine Feuchtigkeitssensoren gefunden' : 'No humidity sensors found'}
                                    </div>
                                )}
                                {filtered.map(s => (
                                    <div key={s.entity_id} style={{
