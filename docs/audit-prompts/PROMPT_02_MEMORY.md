...
## Konkrete Fixes (aus Audit-Durchlauf #2)

Die folgenden 6 Bugs wurden im zweiten Audit-Durchlauf verifiziert und priorisiert:

**BUG 1: Conversation-History zu kurz (KRITISCH)**
- Problem: brain.py get_recent_conversations(limit=3) — nur 3 Nachrichten als Kontext
- Fix: limit=3 → limit=10 an brain.py Zeile ~3080 UND Zeile ~2442
- Verify: grep "get_recent_conversations" brain.py → alle muessen limit=10+ zeigen

**BUG 2: Semantische Fakten NUR bei memory-Intent (KRITISCH)**
- Problem: brain.py _mega_tasks — search_facts/get_facts_by_person nur bei intent=="memory"
- Fix: IMMER in _mega_tasks einfuegen (nicht bedingt):
  ```python
  ...
  ```
- Verify: grep "search_facts\|get_facts_by_person" brain.py → muss in _mega_tasks stehen (unconditional)

**BUG 3: Memory-Intent-Erkennung zu eng**
- Problem: Nur exakter "memory" Intent triggert Fakten-Laden
- Fix: Keywords erweitern: "weisst du", "kennst du", "erinnerst du", "was habe ich", "wer bin ich", "wie heisse ich", "mein Name", "mein Geburtstag", "meine Frau", "mein Mann", "wo wohne ich"

**BUG 4: Memory-Kontext im System-Prompt unklar**
- Problem: _build_memory_context() Header sagt "nutze mit Haltung wie ein alter Bekannter" — LLM versteht nicht dass das SEINE Erinnerungen sind
- Fix: Header zu "DEIN GEDAECHTNIS — folgende Fakten weisst du ueber den User:"

**BUG 5: conversation_memory Prioritaet zu niedrig**
- Problem: brain.py ~2973: priority 3 → wird bei knappem Token-Budget gedroppt
- Fix: priority 3 → priority 1 (Memory ist KERNFUNKTION)

**BUG 6: Doppelter Wort-Filter (KRITISCH)**
- Problem: brain.py (>3 Woerter) UND memory_extractor.py (>=5 Woerter) — doppelter Filter killt die meisten Fakten
- Fix: brain.py Filter auf >2, memory_extractor.py auf >=3
...
Fuer JEDEN Bug:
1. Datei lesen (Read)
2. Alle Aufrufer finden (Grep)
3. Fix implementieren (Edit)
4. Fix verifizieren (Grep/Read)
5. ERST DANN naechster Bug
