...
TEST-DIALOG 1: Einfacher Lichtbefehl
  User: "Mach das Licht an"
  → Tool-Call: set_light(state="on")
  → Jarvis: "Erledigt." (NICHT "Natürlich! Ich schalte das Licht ein!")

TEST-DIALOG 2: Rollladen mit Prozent
  User: "Rollladen auf 50%"
  → Tool-Call: set_cover(position=50)

TEST-DIALOG 3: Mehrere Geräte
  User: "Licht aus und Rollladen runter"
  → Tool-Calls: set_light(state="off") + set_cover(action="close")

TEST-DIALOG 4: Query vs Command
  User: "Wie warm ist es im Wohnzimmer?"
  → Query-Tool (NICHT set_climate!)

TEST-DIALOG 5: Schalter
  User: "Schalte die Steckdose im Büro aus"
  → Tool-Call: set_switch(state="off")

TEST-DIALOG 6: Dimmen
  User: "Dimme das Licht auf 30%"
  → Tool-Call: set_light(brightness=30)

## Erfolgs-Check
□ grep "TOOL-CALLING\|PFLICHT" personality.py → in ersten 10 Zeilen des Templates
□ grep "supports_think_with_tools.*false" settings.yaml → 1 Treffer
□ grep "_deterministic_tool_call" brain.py → deckt Licht, Cover, Climate, Switch ab
□ Tool-Anzahl pro Request von 45+ auf 15 reduziert für Device-Commands
